...
def Main():
    ...
    if (page == 1):
        ...
        perv = "/?page=" + str(page-1)
        next = "#"
    else:
        perv = "/?page=" + str(page - 1)
        next = "/?page=" + str(page + 1)

    return render_template('index.html', params=params, posts=posts, perv=perv, next=next)

@app.route("/home")
def home():
    posts = Posts.query.filter_by().all()[0:params['noofpost']]
...
def contact():
    ...
    if(request.method=='POST'):
        ...
        entry = Contact(name = name, mobile = mobile, email = email, date=datetime.now(), message = message)
        db.session.add(entry)
        db.session.commit()
        mail.send_message('New Mail from site!',
                          sender=email,
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, tagline=tagline, slug=slug, content=content, img_file=image, date=date)
                db.session.add(post)
                db.session.commit()
